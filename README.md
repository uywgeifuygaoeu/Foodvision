...
deployment checkpoint is published as chunked GitHub Release assets. Download, rebuild, and
verify it after cloning:
...
The trained checkpoint is available as chunked GitHub Release assets. Run
`./backend/models/download_model.sh` after cloning so the application can use real inference
without retraining. The Release page URL is listed in `application/model_link.txt`.
